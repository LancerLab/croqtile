...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<A::Node, Ptr<A::Node>>(m, "Node")
      ...
      .def("type_name",
           [](const A::Node& self) { return self.TypeNameString(); });
  ...
  m.def(
      "make_id_expr",
      [](const std::string& name) -> Ptr<A::Expr> {
        return A::MakeIdExpr(pyloc(), name);
      },
      py::arg("name"), "Create an identifier-reference expression.");
  ...
  m.def(
      ...
      py::arg("value"), "Create an integer literal expression.");

  m.def(
      "make_float_expr",
      [](double val) -> Ptr<A::Expr> {
        auto lit = A::Make<A::FloatLiteral>(pyloc(), val);
        return A::Make<A::Expr>(pyloc(), static_cast<Ptr<A::Node>>(lit));
      },
      py::arg("value"), "Create a float literal expression.");

  m.def(
      "make_binary_expr",
      [](const std::string& op, const Ptr<A::Node>& lhs,
  ...
  m.def(
      "make_ternary_expr",
      [](const Ptr<A::Expr>& cond, const Ptr<A::Node>& true_val,
         const Ptr<A::Node>& false_val) -> Ptr<A::Expr> {
        return A::Make<A::Expr>(pyloc(), C::Opcode("?"), cond, true_val,
                                false_val);
      },
  ...
  m.def(
      "make_elemof_expr",
      [](const Ptr<A::Expr>& base, const Ptr<A::Expr>& index) -> Ptr<A::Expr> {
        return A::Make<A::Expr>(pyloc(), C::Opcode(C::Opcode::Kind::ElemOf),
  ...
  py::class_<A::DataType, A::Node, Ptr<A::DataType>>(m, "DataType")
      .def(py::init([](C::BaseType bt) {
           ...
           py::arg("base_type"), "Create a scalar DataType.")
      .def(py::init([](C::BaseType bt,
             ...
             auto mds = A::Make<A::MultiDimSpans>(
                 pyloc(), std::string(""), static_cast<Ptr<A::Node>>(dims));
             return A::Make<A::DataType>(pyloc(), bt,
                                         static_cast<Ptr<A::Node>>(mds));
           }),
  ...
  py::class_<A::Memory, A::Node, Ptr<A::Memory>>(m, "Memory")
      .def(
          py::init([](C::Storage s) { return A::Make<A::Memory>(pyloc(), s); }),
          py::arg("storage"));

  // ---- Parameter / ParamList --------------------------------

  py::class_<A::Parameter, A::Node, Ptr<A::Parameter>>(m, "Parameter")
      .def(py::init([](const Ptr<A::DataType>& type, const std::string& name) {
  ...
  py::class_<A::NamedVariableDecl, A::Node, Ptr<A::NamedVariableDecl>>(
      m, "NamedVariableDecl")
      .def(py::init([](const std::string& name, const Ptr<A::DataType>& type) {
  ...
  py::class_<A::DataAccess, A::Node, Ptr<A::DataAccess>>(m, "DataAccess")
      .def(py::init([](const std::string& name) {
           ...
           py::arg("name"))
      .def(py::init(
               [](const std::string& name, const Ptr<A::MultiValues>& indices) {
  ...
  py::class_<A::Assignment, A::Node, Ptr<A::Assignment>>(m, "Assignment")
      ...
      .def(
          py::init([](const Ptr<A::DataAccess>& da, const Ptr<A::Node>& value) {
  ...
  py::class_<A::ParallelBy, A::Block, Ptr<A::ParallelBy>>(m, "ParallelBy")
      .def("set_body",
           [](A::ParallelBy& self, const Ptr<A::MultiNodes>& stmts) {
             self.stmts = stmts;
           })
  ...
  auto apply_scope = [](Ptr<A::ParallelBy>& pb, const std::string& scope) {
    ...
    if (scope == "block")
      lvl = C::ParallelLevel::BLOCK;
    else if (scope == "group")
      lvl = C::ParallelLevel::GROUP;
    else if (scope == "group-4")
      lvl = C::ParallelLevel::GROUPx4;
    else if (scope == "thread")
      lvl = C::ParallelLevel::THREAD;
    else if (scope == "cluster")
      lvl = C::ParallelLevel::CLUSTER;
  ...
  m.def(
      ...
      [&apply_scope](const std::string& var_name, int bound,
        ...
        auto spv_bounds = A::Make<A::MultiValues>(pyloc(), std::string(", "));
        spv_bounds->Append(p_bound->Clone());
        spv_bounds->SetType(C::MakeITupleType(1));

        auto pb =
            A::Make<A::ParallelBy>(pyloc(), pv, p_bound, spv, spv_bounds, body);
  ...
  m.def(
      ...
      py::arg("scope") = "", "Create a ParallelBy node with expression bound.");
  ...
  m.def(
      ...
      [](const std::vector<std::pair<std::string, int>>& bindings,
        ...
        for (auto& [name, bound] : bindings) {
          ...
          auto matchers = A::Make<A::MultiValues>(pyloc(), std::string(", "));
  ...
  m.def(
      ...
      py::arg("targets"), "Create a Wait node for async DMA/TMA futures.");
  ...
  m.def(
      "make_chunkat_view",
      [](const std::string& name, const std::vector<Ptr<A::Node>>& shape,
  ...
  m.def(
      ...
      [](const std::string& name, const std::vector<Ptr<A::Node>>& shape,
        ...
        auto ss = A::Make<A::SOP::SubSpan>(pyloc(), ss_shape, ss_idx, ss_steps);
        std::vector<std::shared_ptr<A::SpannedOperation>> ops;
  ...
  m.def(
      "make_dma_copy",
      [](const std::string& future_name, const Ptr<A::Node>& from,
         const Ptr<A::Node>& to, bool async, bool is_tma,
         int swizzle) -> Ptr<A::DMA> {
        A::DMAAsync da(async);
        ...
        return A::Make<A::DMA>(pyloc(), ".copy", future_name, from, to, da,
                               attr, is_tma);
      },
      py::arg("future") = "", py::arg("from") = nullptr,
      py::arg("to") = nullptr, py::arg("async") = false,
      py::arg("is_tma") = false, py::arg("swizzle") = 0,
  ...
  m.def(
      ...
      [](const std::string& future_name) -> Ptr<A::DMA> {
        A::DMAAsync da(false);
        return A::Make<A::DMA>(pyloc(), ".any", future_name, nullptr, nullptr,
                               da);
      },
      py::arg("future"), "Create a DMA.any placeholder future.");
  ...
  m.def(
      ...
      [](double val, const std::string& buffer_name,
        ...
        Ptr<A::Expr> buffer =
            buffer_name.empty() ? nullptr : A::MakeIdExpr(pyloc(), buffer_name);
        bool is_decl = (buffer == nullptr);
        auto op = std::make_shared<A::MMAOperation>(
            buffer,
            std::static_pointer_cast<A::Expr>(A::Make<A::Expr>(
                pyloc(), static_cast<Ptr<A::Node>>(fill_expr))),
            is_decl, elem_type);
        return A::Make<A::MMA>(pyloc(), op);
      },
      py::arg("value") = 0.0, py::arg("buffer") = "",
  ...
  m.def(
      ...
      [](const Ptr<A::ChunkAt>& expr, int swizzle) -> Ptr<A::MMA> {
        ...
        if (swizzle == 32)
          ...
          sm = C::SwizMode::B128;
        auto op =
            std::make_shared<A::MMAOperation>(expr, nullptr, false, sm, false);
        return A::Make<A::MMA>(pyloc(), op);
      },
  ...
  m.def(
      "make_mma_exec",
      [](const std::string& method, const Ptr<A::Expr>& acc,
         const Ptr<A::Expr>& lhs, const Ptr<A::Expr>& rhs) -> Ptr<A::MMA> {
  ...
  m.def(
      "make_mma_store",
      [](const Ptr<A::Expr>& buffer, const Ptr<A::ChunkAt>& target,
         bool transpose) -> Ptr<A::MMA> {
        auto op = std::make_shared<A::MMAOperation>(buffer, target, transpose);
        return A::Make<A::MMA>(pyloc(), op);
      },
      py::arg("buffer"), py::arg("target"), py::arg("transpose") = false,
      "Create MMA store: mma.store mc, output.chunkat(m, n).");

  m.def(
      "make_mma_store_var",
      [](const Ptr<A::Expr>& buffer, const std::string& target_name,
         bool transpose) -> Ptr<A::MMA> {
      ...
      py::arg("buffer"), py::arg("target_name"), py::arg("transpose") = false,
  ...
  m.def(
      ...
      [](const std::string& func_name,
        ...
        auto call = A::Make<A::Call>(
            pyloc(), id, mv, A::Call::BIF | A::Call::ARITH | A::Call::EXPR);
        return A::Make<A::Expr>(pyloc(), static_cast<Ptr<A::Node>>(call));
      },
  ...
  m.def(
      ...
      py::arg("args"), "Create a println statement: println(a, b, ...).");
  ...
  m.def(
      ...
      py::arg("a"), py::arg("b"), "Create a Swap (Rotate) node: swap(a, b).");
  ...
  m.def(
      "make_function",
      [](const std::string& name, const Ptr<A::DataType>& ret_type,
      ...
      py::arg("body") = nullptr, "Create a ChoreoFunction.");

  // ---- Program ----------------------------------------------

  py::class_<A::Program, A::Block, Ptr<A::Program>>(m, "Program")
      .def(py::init([]() { return A::Make<A::Program>(pyloc()); }))
      .def("append", [](A::Program& self,
                        const Ptr<A::Node>& node) { self.stmts->Append(node); })
      .def("print_ast",
           [](const A::Program& self) {
             std::ostringstream os;
             self.Print(os);
             return os.str();
           })
      .def("count", [](const A::Program& self) { return self.stmts->Count(); });

  // ---- CppSourceCode ----------------------------------------

  py::class_<A::CppSourceCode, A::Node, Ptr<A::CppSourceCode>>(m,
                                                               "CppSourceCode")
      .def(py::init([](const std::string& code) {
             return A::Make<A::CppSourceCode>(pyloc(), code,
                                              A::CppSourceCode::Host);
           }),
           py::arg("code"));
  ...
  m.def(
      ...
      [](const std::string& target_name, const std::string& arch) {
        ...
        if (!tgt) throw std::runtime_error("Unknown target: " + target_name);
  ...
  m.def(
      "set_no_codegen", [](bool v) { C::CCtx().SetNoCodegen(v); },
      py::arg("value") = true);
  ...
  m.def(
      ...
      [](const A::Program& prog, const std::string& path) {
        ...
        if (!ofs) throw std::runtime_error("Cannot write to: " + path);
        prog.Print(ofs);
        ofs.close();
      },
  ...
  m.def(
      ...
      py::arg("program"), "Print the AST tree as a string.");
