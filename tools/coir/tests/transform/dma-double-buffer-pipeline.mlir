...
coir.kernel @double_buffer_full(
  ...
  // CHECK: %[[FB_DESC:.*]] = coir.dma.const.desc %{{.*}}, %[[BUF_B]] {kind = #coir.dma_kind<slice>, src_tiled, tile_shape = array<i64: 256>}
  // CHECK: %[[FB_PRE:.*]] = coir.dma.prefetch.desc %[[FB_DESC]]

  // --- Design 3b: output copy's const.desc + prefetch HOISTED above loop ---
  // CHECK: %[[OUT_DESC:.*]] = coir.dma.const.desc %[[BUF_A]], %{{.*}} {dst_tiled, kind = #coir.dma_kind<slice>, tile_shape = array<i64: 256>}
