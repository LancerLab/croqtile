  ...
  if (auto *expr = dyn_cast<AST::Expr>(&n)) {
    ...
    if (expr->GetForm() == AST::Expr::Unary) {
      ...
      if (op == Op::Sub) {
        ...
        if (isFloat) {
          auto zero = COIR_CONSTANT_FLOAT(builder, loc, llvm::APFloat(0.0f), mlir::cast<mlir::FloatType>(operand.getType()));
  ...
  for (auto &op : asmStmt.outputOperands) {
    ...
    if (!val && op->constraint[0] == '=' && !inVals.empty()) {
      ...
      if (mlir::isa<mlir::IntegerType>(ty)) {
        ...
        auto zero = COIR_CONSTANT_FLOAT(builder, loc, llvm::APFloat(0.0f), mlir::cast<mlir::FloatType>(ty));
  ...
  for (unsigned i = 0; i < asmStmt.outputOperands.size() && i < outVals.size();
    ...
    if (auto *id =
      ...
      if (da->AccessElement()) {
        ...
        if (tensorVal) {
          auto tty =
              mlir::dyn_cast<coir::TensorType>(tensorVal.getType());
          if (tty) {
            llvm::SmallVector<mlir::Value> idxVals;
            for (auto &idx : da->GetIndices()) {
              mlir::Value v = EmitExpr(*idx);
              if (!v) {
                if (auto *idNode =
                        dyn_cast<AST::Identifier>(idx.get()))
                  v = LookupValue(idNode->name);
              }
              if (v && !mlir::isa<mlir::IndexType>(v.getType()))
                v = builder.create<mlir::arith::IndexCastOp>(
            ...
            builder.create<coir::TensorStoreElemOp>(
                loc, asmOp.getResult(i), tensorVal, idxVals);
          }
        }
      }
    }
