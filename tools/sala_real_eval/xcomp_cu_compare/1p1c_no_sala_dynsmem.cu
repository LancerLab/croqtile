...
__global__ __launch_bounds__(256, 2) void __choreo_device_matmul(f16 * lhs, f16 * rhs, f16 * output, unsigned K, unsigned M, unsigned N, const __grid_constant__ CUtensorMap __choreo_tma_0_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_1_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_2_tensor_map) {
  ...
  // No-SALA layout: sequential allocation, no overlap (like CUTLASS struct)
  // lhs_load_s: [0,        32768)   = 32 KB
  // rhs_load_s: [32768,    98304)   = 64 KB
  // output_s:   [98304,   114688)   = 16 KB
  // total:      114688 bytes = 112 KB
  [[maybe_unused]] auto __choreo_device_matmul__ring__ = nullptr;
  { // parallel-by: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:53.12
  ...
  __syncthreads();
  // No-SALA: sequential layout, each buffer at a unique non-overlapping offset
  f16* lhs_load_s = (f16*)(__nosala_buf__);
  f16* rhs_load_s = (f16*)(__nosala_buf__ + sizeof(f16) * 16384);
  f16* output_s   = (f16*)(__nosala_buf__ + sizeof(f16) * (16384 + 32768));
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:62.7
  if ((__choreo_vg4id_x == 0 && __choreo_vtid_x == 0)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:63.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:63.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        ...
        cde::cp_async_bulk_tensor_2d_global_to_shared((lhs_load_s + ((__iv_iv_k % 4 * 4096))), &__choreo_tma_0_tensor_map, (__iv_iv_k * 64), (blockIdx.x * 64), full[stage]);
        cde::cp_async_bulk_tensor_2d_global_to_shared((rhs_load_s + ((__iv_iv_k % 4 * 8192))), &__choreo_tma_1_tensor_map, (K / ((K + 63) / 64) * __iv_iv_k), (N / ((N + 127) / 128) * blockIdx.y), full[stage]);
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:72.7
  if ((__choreo_vg4id_x == 1)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:73.9
    {
      int __iv_s = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:73.9
    ...
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:76.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:76.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        auto stage = __iv_iv_k % 4;
        // wait event(barrier)  (full elemof stage) 
        full[stage].wait(full[stage].arrive());
        // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:79.11
        {
          int __iv_iv_warp = 0;
          // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:79.11
          warpgroup_fence_operand(mc);
          warpgroup_arrive();
          for (__iv_iv_warp = 0; __iv_iv_warp < 4; ++__iv_iv_warp) {
            f16* ma_smem_ptr = (f16*)((__iv_iv_warp * 16 + __iv_iv_k % 4 * 4096 + lhs_load_s));
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:89.7
  if ((__choreo_vg4id_x == 1)) {
    ...
    future __choreo_anon_fut__0("", 91, 9, output);
...
void matmul(const choreo::spanned_view<choreo::f16, 2> & lhs, const choreo::spanned_view<choreo::f16, 2> & rhs, const choreo::spanned_view<choreo::f16, 2> & output) {
  ...
  choreo::runtime_check(((static_cast<long long>(M) + 63LL) / 64LL > 0LL), "The 1st bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:53.13");
  choreo::runtime_check(((static_cast<long long>(N) + 127LL) / 128LL > 0LL), "The 2nd bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:53.22");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:63.27");
  choreo::runtime_check((static_cast<long long>(K) / ((static_cast<long long>(K) + 63LL) / 64LL) * (static_cast<long long>(N) / ((static_cast<long long>(N) + 127LL) / 128LL)) <= 8192LL), "DMA to-buffer is too small (((::matmul::K / ((::matmul::K + 63) / 64)) * (::matmul::N / ((::matmul::N + 127) / 128))) > 8192), Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:67.11");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_split.co:76.27");
  ...
  constexpr int __nosala_smem_bytes = 114688 + (1024 - 1);
