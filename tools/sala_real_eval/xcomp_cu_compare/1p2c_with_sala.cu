...
#define DEFAULT_K 8192

__global__ __launch_bounds__(384, 1) void __choreo_device_matmul(f16 * lhs, f16 * rhs, f16 * output, unsigned K, unsigned M, unsigned N, const __grid_constant__ CUtensorMap __choreo_tma_0_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_1_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_2_tensor_map) {
  ...
  { // parallel-by: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:20.12
  ...
  auto anon_2 = (unsigned char*)__choreo_device_matmul__runtime_shared_buffer__;
  ...
  __syncthreads();
  f16* lhs_s = (f16*)(anon_2 + 49152);
  f16* rhs_s = (f16*)(anon_2 + 0);
  [[maybe_unused]] auto __choreo_vg4id_x = threadIdx.x / 128;
  [[maybe_unused]] auto __choreo_vtid_x = threadIdx.x % 128;
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:25.7
  if ((__choreo_vg4id_x == 0 && __choreo_vtid_x == 0)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:26.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:26.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        ...
        cde::cp_async_bulk_tensor_2d_global_to_shared(lhs_s + (stage)*(8192), &__choreo_tma_0_tensor_map, (__iv_iv_k * 64), (blockIdx.x * 128), full[stage]);
        cde::cp_async_bulk_tensor_2d_global_to_shared(rhs_s + (stage)*(12288), &__choreo_tma_1_tensor_map, (__iv_iv_k * 64), (blockIdx.y * 192), full[stage]);
        // trigger event(barrier)  (full elemof stage) 
        (void)cuda::device::barrier_arrive_tx(full[stage], 1, (16384) + (24576));
      } // iv_k
      __iv_iv_k = 0;
    }
  } // end inthreads
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:37.7
  if ((__choreo_vg4id_x >= 1)) {
    int cidx = __choreo_vg4id_x - 1;
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:39.9
    {
      int __iv_s = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:39.9
    ...
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:42.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:42.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        auto stage = __iv_iv_k % 2;
        // wait event(barrier)  (full elemof stage) 
        full[stage].wait(full[stage].arrive());
        // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:45.11
        {
          int __iv_iv_wk = 0;
          // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:45.11
          warpgroup_fence_operand(mc);
          warpgroup_arrive();
          for (__iv_iv_wk = 0; __iv_iv_wk < 4; ++__iv_iv_wk) {
            f16* ma_smem_ptr = (f16*)(((stage)*(8192) + (__iv_iv_wk * 16 + (__choreo_vg4id_x - 1) * 4096) + lhs_s));
            uint64_t desc_ma = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(ma_smem_ptr);
            f16* mb_smem_ptr = (f16*)(((stage)*(12288) + __iv_iv_wk * 16 + rhs_s));
            uint64_t desc_mb = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(mb_smem_ptr);
            cute::SM90::GMMA::MMA_64x192x16_F16F16F16_SS<static_cast<cute::SM90::GMMA::Major>(0), static_cast<cute::SM90::GMMA::Major>(0)>::fma(desc_ma, desc_mb, mc[0], mc[1], mc[2], mc[3], mc[4], mc[5], mc[6], mc[7], mc[8], mc[9], mc[10], mc[11], mc[12], mc[13], mc[14], mc[15], mc[16], mc[17], mc[18], mc[19], mc[20], mc[21], mc[22], mc[23], mc[24], mc[25], mc[26], mc[27], mc[28], mc[29], mc[30], mc[31], mc[32], mc[33], mc[34], mc[35], mc[36], mc[37], mc[38], mc[39], mc[40], mc[41], mc[42], mc[43], mc[44], mc[45], mc[46], mc[47]);
          } // iv_wk
          __iv_iv_wk = 0;
        }
  ...
  __syncthreads();
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:56.7
  if ((__choreo_vg4id_x >= 1)) {
    auto cidx = __choreo_vg4id_x - 1;
    f16* output_s = (f16*)(anon_2 + 0);
    warpgroup_commit_batch();
    warpgroup_wait<0>();
    warpgroup_fence_operand(mc);
    auto __shape1_output_s = cute::make_shape(cute::Int<64>{}, cute::Int<192>{});
    auto __stride1_output_s = cute::make_stride(cute::Int<192>{}, cute::Int<1>{});
    auto __layout1_output_s = cute::make_layout(__shape1_output_s, __stride1_output_s);
    auto __tensor1_output_s = cute::make_tensor(cute::make_smem_ptr<f16>((f16*)output_s + (cidx)*(12288) + 0), __layout1_output_s);
    store_fragment_d<CUTE_WGMMA_M64K16, 192>(__tensor1_output_s, reinterpret_cast<f16*>(mc));
    asm volatile("bar.sync 15, 128;\n" ::: "memory");
    [[maybe_unused]] auto anon_1 = (blockIdx.x * 2 + (__choreo_vg4id_x - 1));
    future __choreo_anon_fut__0("", 60, 9, output);
    __choreo_anon_fut__0.is_tma = true;
    __choreo_anon_fut__0.set_atom(&choreo_copy_atom_t_0);
    cde::fence_proxy_async_shared_cta();
    asm volatile("bar.sync 15, 128;\n" ::: "memory");
    if (__CHOREO_GROUPX4_SINGLE__ && __choreo_vg4id_x == 1) {
      cde::cp_async_bulk_tensor_2d_shared_to_global(&__choreo_tma_2_tensor_map, (blockIdx.y * 192), (anon_1 * 64), output_s + (cidx)*(12288));
...
void matmul(const choreo::spanned_view<choreo::f16, 2> & lhs, const choreo::spanned_view<choreo::f16, 2> & rhs, const choreo::spanned_view<choreo::f16, 2> & output) {
  ...
  choreo::runtime_check(((static_cast<long long>(M) + 127LL) / 128LL > 0LL), "The 1st bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:20.13");
  choreo::runtime_check(((static_cast<long long>(N) + 191LL) / 192LL > 0LL), "The 2nd bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:20.22");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:26.27");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p2c_split.co:42.27");
  uint64_t __choreo_tma_0_shape[] = {K, M};
  uint64_t __choreo_tma_0_strides[] = {(K * 2)};
  uint32_t __choreo_tma_0_box_shape[] = {(uint32_t)(64), (uint32_t)(128)};
  ...
  uint32_t __choreo_tma_1_box_shape[] = {(uint32_t)(64), (uint32_t)(192)};
  ...
  uint32_t __choreo_tma_2_box_shape[] = {(uint32_t)(192), (uint32_t)(64)};
  ...
  dim3 __matmul_gdims0(((M + 127) / 128), ((N + 191) / 192), 1);
