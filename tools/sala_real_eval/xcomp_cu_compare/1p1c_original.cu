...
__global__ __launch_bounds__(256, 2) void __choreo_device_matmul(f16 * lhs, f16 * rhs, f16 * output, unsigned K, unsigned M, unsigned N, const __grid_constant__ CUtensorMap __choreo_tma_0_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_1_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_2_tensor_map) {
  ...
  { // parallel-by: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:53.12
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:60.7
  if ((__choreo_vg4id_x == 0 && __choreo_vtid_x == 0)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:61.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:61.9
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:70.7
  if ((__choreo_vg4id_x == 1)) {
    unsigned int mc[32];
    uint32_t __frag_init_val0 = broadcast_to_u32(choreo::f32_to_f16(0.000000f));
    for (int idx = 0; idx < 32; ++idx)
      mc[idx] = __frag_init_val0;
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:72.9
    {
      int __iv_s = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:72.9
    ...
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:75.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:75.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        auto stage = __iv_iv_k % 4;
        // wait event(barrier)  (full elemof stage) 
        full[stage].wait(full[stage].arrive());
        // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:78.11
        {
          int __iv_iv_warp = 0;
          // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:78.11
    ...
    future __choreo_anon_fut__0("", 87, 9, output);
...
void matmul(const choreo::spanned_view<choreo::f16, 2> & lhs, const choreo::spanned_view<choreo::f16, 2> & rhs, const choreo::spanned_view<choreo::f16, 2> & output) {
  ...
  choreo::runtime_check(((static_cast<long long>(M) + 63LL) / 64LL > 0LL), "The 1st bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:53.13");
  choreo::runtime_check(((static_cast<long long>(N) + 127LL) / 128LL > 0LL), "The 2nd bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:53.22");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:61.27");
  choreo::runtime_check((static_cast<long long>(K) / ((static_cast<long long>(K) + 63LL) / 64LL) * (static_cast<long long>(N) / ((static_cast<long long>(N) + 127LL) / 128LL)) <= 8192LL), "DMA to-buffer is too small (((::matmul::K / ((::matmul::K + 63) / 64)) * (::matmul::N / ((::matmul::N + 127) / 128))) > 8192), Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:65.11");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p1c_original.co:75.27");
