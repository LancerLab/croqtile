...
__global__ __launch_bounds__(512, 1) void __choreo_device_matmul(f16 * lhs, f16 * rhs, f16 * output, unsigned K, unsigned M, unsigned N, const __grid_constant__ CUtensorMap __choreo_tma_0_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_1_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_2_tensor_map) {
  extern __shared__ char __nosala_shared_buf__[];
  auto __nosala_buf__ = reinterpret_cast<char*>(aligned_up_ptr<1024 * 8>(__nosala_shared_buf__));
  [[maybe_unused]] auto __choreo_device_matmul__ring__ = nullptr;
  { // parallel-by: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:53.12
  ...
  __syncthreads();
  f16* lhs_load_s = (f16*)(__nosala_buf__ + 0);
  f16* rhs_load_s = (f16*)(__nosala_buf__ + 49152);
  f16* output_s   = (f16*)(__nosala_buf__ + 81920);
  ...
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:62.7
  if ((__choreo_vg4id_x == 0 && __choreo_vtid_x == 0)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:63.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:63.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        ...
        auto lhs_s_ref = (__iv_iv_k % 2 * 12288 + lhs_load_s);
        auto rhs_s_ref = (__iv_iv_k % 2 * 8192 + rhs_load_s);
        cde::cp_async_bulk_tensor_2d_global_to_shared(lhs_s_ref, &__choreo_tma_0_tensor_map, (__iv_iv_k * 64), (blockIdx.x * 192), full[stage]);
        cde::cp_async_bulk_tensor_2d_global_to_shared(rhs_s_ref, &__choreo_tma_1_tensor_map, (K / ((K + 63) / 64) * __iv_iv_k), (N / ((N + 127) / 128) * blockIdx.y), full[stage]);
        // trigger event(barrier)  (full elemof stage) 
        (void)cuda::device::barrier_arrive_tx(full[stage], 1, (24576) + (16384));
      } // iv_k
      __iv_iv_k = 0;
    }
  } // end inthreads
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:74.7
  if ((__choreo_vg4id_x > 0)) {
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:75.9
    {
      int __iv_s = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:75.9
    ...
    // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:78.9
    {
      int __iv_iv_k = 0;
      // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:78.9
      for (__iv_iv_k = 0; __iv_iv_k < ((K + 63) / 64); ++__iv_iv_k) {
        auto stage = __iv_iv_k % 2;
        // wait event(barrier)  (full elemof stage) 
        full[stage].wait(full[stage].arrive());
        // with-in: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:81.11
        {
          int __iv_iv_warp = 0;
          // foreach: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:81.11
          warpgroup_fence_operand(mc);
          warpgroup_arrive();
          for (__iv_iv_warp = 0; __iv_iv_warp < 4; ++__iv_iv_warp) {
            [[maybe_unused]] auto anon_1 = (__choreo_vg4id_x + __iv_iv_k % 2 * 3 - 1);
            auto ma_ref = (anon_1 * 4096 + __iv_iv_warp * 16 + lhs_load_s);
            auto mb_ref = (__iv_iv_warp * 16 + __iv_iv_k % 2 * 8192 + rhs_load_s);
            f16* ma_smem_ptr = (f16*)(ma_ref);
            uint64_t desc_ma = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(ma_smem_ptr);
            f16* mb_smem_ptr = (f16*)(mb_ref);
            uint64_t desc_mb = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(mb_smem_ptr);
            cute::SM90::GMMA::MMA_64x128x16_F16F16F16_SS<static_cast<cute::SM90::GMMA::Major>(0), static_cast<cute::SM90::GMMA::Major>(0)>::fma(desc_ma, desc_mb, mc[0], mc[1], mc[2], mc[3], mc[4], mc[5], mc[6], mc[7], mc[8], mc[9], mc[10], mc[11], mc[12], mc[13], mc[14], mc[15], mc[16], mc[17], mc[18], mc[19], mc[20], mc[21], mc[22], mc[23], mc[24], mc[25], mc[26], mc[27], mc[28], mc[29], mc[30], mc[31]);
          } // iv_warp
          __iv_iv_warp = 0;
        }
  ...
  __syncthreads();
  // inthreads: Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:95.7
  if ((__choreo_vg4id_x > 0)) {
    [[maybe_unused]] auto anon_2 = __choreo_vg4id_x - 1;
    ...
    store_fragment_d<CUTE_WGMMA_M64K16, 128>(__tensor1_output_s, reinterpret_cast<f16*>(mc));
  ...
  __syncthreads();
  if (__CHOREO_BLOCK_SINGLE__) {
    cde::cp_async_bulk_tensor_2d_shared_to_global(&__choreo_tma_2_tensor_map, (blockIdx.y * 128), (blockIdx.x * 192), output_s);
...
void matmul(const choreo::spanned_view<choreo::f16, 2> & lhs, const choreo::spanned_view<choreo::f16, 2> & rhs, const choreo::spanned_view<choreo::f16, 2> & output) {
  ...
  choreo::runtime_check(((static_cast<long long>(M) + 191LL) / 192LL > 0LL), "The 1st bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:53.13");
  choreo::runtime_check(((static_cast<long long>(N) + 127LL) / 128LL > 0LL), "The 2nd bound item of parallelby is invalid: should be greater than 0, Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:53.22");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:63.27");
  choreo::runtime_check((static_cast<long long>(K) / ((static_cast<long long>(K) + 63LL) / 64LL) * (static_cast<long long>(N) / ((static_cast<long long>(N) + 127LL) / 128LL)) <= 8192LL), "DMA to-buffer is too small (((::matmul::K / ((::matmul::K + 63) / 64)) * (::matmul::N / ((::matmul::N + 127) / 128))) > 8192), Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:69.11");
  choreo::runtime_check(((static_cast<long long>(K) + 63LL) / 64LL != 0LL), "zero is detected for the 1st dim of the mdspan inside the with-in statement, Documents/cgo-paper/signal-aware-liveness/bench_1p3c_split.co:78.27");
  uint64_t __choreo_tma_0_shape[] = {K, M};
  uint64_t __choreo_tma_0_strides[] = {(K * 2)};
  uint32_t __choreo_tma_0_box_shape[] = {(uint32_t)(64), (uint32_t)(192)};
  ...
  uint32_t __choreo_tma_1_box_shape[] = {(uint32_t)((K / ((K + 63) / 64))), (uint32_t)((N / ((N + 127) / 128)))};
  ...
  uint32_t __choreo_tma_2_box_shape[] = {(uint32_t)(128), (uint32_t)(192)};
  ...
  dim3 __matmul_gdims0(((M + 191) / 192), ((N + 127) / 128), 1);
  dim3 __matmul_bdims0(512, 1, 1);
  constexpr int __nosala_smem_bytes = 131072 + (1024 - 1);
  cudaFuncSetAttribute(__choreo_device_matmul, cudaFuncAttributeMaxDynamicSharedMemorySize, __nosala_smem_bytes);
  __choreo_device_matmul<<<__matmul_gdims0, __matmul_bdims0, __nosala_smem_bytes>>>(lhs.data(), rhs.data(), output.data(), K, M, N, __choreo_tma_0_tensor_map, __choreo_tma_1_tensor_map, __choreo_tma_2_tensor_map);
