# Choreo - An eDSL for Dataflow Programming
Choreo is a low-level embedded Domain Specific Language (eDSL) works within device-level C++ APIs. It is designed to facilitate the manipulating of DMA entities of heterogenous hardware accelerators and save the mind-set cost of programmers data movement.

## Choreo and Device-Level C++
Choreo has close relationship to the device-level C++ programming, which is usually provided by the hardware vendor. In current implementation, Choreo performs source-to-source translation to convert choreo-c++ programs to vendor-provided APIs (like factor, topscc). Whiling turning higher level abstraction of dataflow functions into corresponding low level C++API calls, Choreo also glues the kernels, the host C++ program source code into single program, and drive all the compilation processes of heterogenous programs for easy use.
