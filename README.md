...
Choreo is a low-level Embedded Domain Specific Language (EDSL) for C++ specifically engineered to program data movement entities like Direct-Memory-Accesses (DMA), which traditionally focus on hardware configuration rather than the data itself. This language facilitates the programming of data flow across a hardware’s memory hierarchy, where at each level, data is partitioned into smaller chunks that are positioned closer to the processing elements. By making data tiling and flowing more manageable, Choreo pioneers the **'TileFlow' programming paradigm**. This new approach aims to significantly streamline the engineering tasks involved, particularly for professionals developing **high-performance machine learnig kernels** on heterogeneous/specific hardware.

## Choreo and Device-Level C++
Choreo has a close relationship with device-level C++ programming languages and APIs, typically provided by hardware vendors. In its current implementation, Choreo performs **source-to-source translation** to convert *choreo-c++* programs into vendor-supported C++ language entities and APIs (such as Factor, Topscc, and CUDA).

Beyond translating higher-level abstractions of *TileFlow* functions into corresponding low-level C++ API calls, Choreo also integrates the kernels and host code into executables. As a result, the Choreo compiler **can perform end-to-end compilation tasks** when the vendor-provided device-level C++ compiler is properly configured.
