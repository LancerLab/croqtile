...
__global__ void __choreo_device_spmm(f16 * lhs_packed, unsigned int * lhs_meta, f16 * rhs, f16 * output, const __grid_constant__ CUtensorMap __choreo_tma_0_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_1_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_2_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_3_tensor_map, const __grid_constant__ CUtensorMap __choreo_tma_4_tensor_map) {
  ...
  if ((__choreo_vg4id_x == 0) && __CHOREO_GROUPX4_SINGLE__) {
    // with-in: /home/albert/workspace-choreo/choreo-cursor/benchmark/performance/gemm_sp/gemm_sp_f16_iter130_f16accum.co:50.9
    {
      ...
      for (__iv_iv_k = 0; __iv_iv_k < 64; ++__iv_iv_k) {
        ...
        choreo::tma_load_2d_shared_cluster_global_mbarrier((void*)(lhs_load_s + ((__iv_iv_k % 2 * 8192))), (const void*)&__choreo_tma_0_tensor_map, (uint64_t*)&(full[stage]), (__iv_iv_k * 64), (blockIdx.x * 128));
        auto anon_1 = 2 * __iv_iv_k;
        choreo::tma_load_2d_shared_cluster_global_mbarrier((void*)(rhs_load_s + ((__iv_iv_k % 2 * 32768))), (const void*)&__choreo_tma_1_tensor_map, (uint64_t*)&(full[stage]), (__iv_iv_k * 128), (blockIdx.y * 256));
        auto anon_2 = 2 * __iv_iv_k + 1;
        choreo::tma_load_2d_shared_cluster_global_mbarrier((void*)(rhs_load_s + ((__iv_iv_k % 2 * 32768 + 64))), (const void*)&__choreo_tma_2_tensor_map, (uint64_t*)&(full[stage]), ((__iv_iv_k * 2 + 1) * 64), (blockIdx.y * 256));
        choreo::tma_load_2d_shared_cluster_global_mbarrier((void*)(meta_s + (stage * 512)), (const void*)&__choreo_tma_4_tensor_map, (uint64_t*)&(full[stage]), ((__iv_iv_k * 2) & ~3), (blockIdx.x * 64));
        // trigger event(barrier)  (full elemof stage) 
        (void)cuda::device::barrier_arrive_tx(full[stage], 1, (16384) + (32768) + (32768) + (2048));
      } // iv_k
      __iv_iv_k = 0;
    }
  } // end inthreads
    ...
    // with-in: /home/albert/workspace-choreo/choreo-cursor/benchmark/performance/gemm_sp/gemm_sp_f16_iter130_f16accum.co:73.9
    {
      ...
      // foreach: /home/albert/workspace-choreo/choreo-cursor/benchmark/performance/gemm_sp/gemm_sp_f16_iter130_f16accum.co:73.9
      {
        ...
        for (__iv_iv_k = 0; __iv_iv_k < 64; ++__iv_iv_k) {
          ...
          int smem_stage_off = stage;

          uint64_t desc_ma[4], desc_mb[4];
          #pragma unroll
          for (int w = 0; w < 4; ++w) {
            f16* ma_smem_ptr = (f16*)((w * 16 + smem_stage_off * 8192 + consumer_off) + lhs_load_s);
            desc_ma[w] = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(ma_smem_ptr);
            f16* mb_smem_ptr = (f16*)((w * 32 + smem_stage_off * 32768) + rhs_load_s);
            desc_mb[w] = wgmma_make_smem_desc<WGMMA_MajorOrder::K_MAJOR, WGMMA_Swizzle::B128>(mb_smem_ptr);
          }
          ...
          if (__sp_active) {
            int col_off = (__iv_iv_k * 2) & 3;
            const uint2* meta_ptr = reinterpret_cast<const uint2*>(&meta_s[stage * 512 + local_packed_row * 8 + col_off]);
  ...
  __syncthreads();
  if (__CHOREO_BLOCK_SINGLE__) {
    cde::cp_async_bulk_tensor_2d_shared_to_global(&__choreo_tma_3_tensor_map, (blockIdx.y * 256), (blockIdx.x * 128), output_s);
