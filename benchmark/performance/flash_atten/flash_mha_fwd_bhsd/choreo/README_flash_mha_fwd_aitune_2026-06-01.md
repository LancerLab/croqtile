...
EXTRA_NVCC_FLAGS="--use_fast_math" bash bench.sh \
  --kernel flash_mha_fwd_aitune_2026-06-01_iter038.co --gpu 1
```

`bench.sh` compiles with `./choreo`, patches `launch_bounds(256,1)` to `(256,2)`,
runs verification and timing. Expect `Test Passed` after sampled numerical checks.
...
| `EXTRA_NVCC_FLAGS` | (none) | Pass `--use_fast_math` for best perf |
