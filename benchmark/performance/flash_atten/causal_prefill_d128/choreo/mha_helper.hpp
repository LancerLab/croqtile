...
struct TensorViews {
  choreo::spanned_view<choreo::bf16, 4> Q;
  choreo::spanned_view<choreo::bf16, 4> K;
  choreo::spanned_view<choreo::bf16, 4> V;
  choreo::spanned_view<choreo::bf16, 4> O;
};
...
inline float read_bshd(const choreo::bf16* data, size_t b, size_t seq,
                       size_t h, size_t d, size_t seq_len, size_t H,
                       size_t head_dim) {
  return choreo::to_f32(
      data[bshd_index(b, seq, h, d, seq_len, H, head_dim)]);
}

inline void fill_random_bshd(choreo::spanned_data<choreo::bf16, 4>& tensor,
                             float lo, float hi, std::mt19937& gen) {
  std::uniform_real_distribution<float> dist(lo, hi);
  const size_t n = tensor.shape()[0] * tensor.shape()[1] * tensor.shape()[2] *
                   tensor.shape()[3];
  for (size_t i = 0; i < n; ++i) {
    tensor.data()[i] = choreo::bf16(dist(gen));
  }
}
...
inline float reference_output(const choreo::bf16* Q, const choreo::bf16* K,
                              const choreo::bf16* V, int b, int h, int qi,
...
inline bool verify_output(const BenchConfig& cfg, const choreo::bf16* Q_h,
                          const choreo::bf16* K_h, const choreo::bf16* V_h,
                          const choreo::bf16* O_h, size_t B, size_t H,
  ...
  for (size_t idx = 0; idx < total && checked < num_samples; idx += stride) {
    ...
    if (err > tol) {
      ++failed;
      if (failed <= 5) {
        std::cout << "  FAIL at [b=" << b << " q=" << qi << " h=" << h
                  << " d=" << di << "] got=" << got << " ref=" << ref
                  << " err=" << err << " tol=" << tol << "\n";
      }
    }
    ++checked;
...
struct DeviceBuffers {
  choreo::bf16* q_d = nullptr;
  choreo::bf16* k_d = nullptr;
  choreo::bf16* v_d = nullptr;
  choreo::bf16* o_d = nullptr;
...
inline TensorViews upload_tensors(
    const choreo::spanned_data<choreo::bf16, 4>& Q_h,
    const choreo::spanned_data<choreo::bf16, 4>& K_h,
    const choreo::spanned_data<choreo::bf16, 4>& V_h,
    choreo::spanned_data<choreo::bf16, 4>& O_h, DeviceBuffers& dev) {
  ...
  const size_t q_bytes = B * Q_SEQ * H * head_dim * sizeof(choreo::bf16);
  const size_t kv_bytes = B * KV_SEQ * H * head_dim * sizeof(choreo::bf16);
  ...
  return TensorViews{
      choreo::make_spanview<choreo::bf16, 4>(dev.q_d,
                                             {B, Q_SEQ, H, head_dim}),
      choreo::make_spanview<choreo::bf16, 4>(dev.k_d,
                                             {B, KV_SEQ, H, head_dim}),
      choreo::make_spanview<choreo::bf16, 4>(dev.v_d,
                                             {B, KV_SEQ, H, head_dim}),
      choreo::make_spanview<choreo::bf16, 4>(dev.o_d,
                                             {B, Q_SEQ, H, head_dim}),
  };
}

inline void download_output(choreo::spanned_data<choreo::bf16, 4>& O_h,
                            choreo::bf16* o_d) {
