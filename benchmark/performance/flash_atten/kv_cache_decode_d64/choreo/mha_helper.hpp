...
// timing, and TFLOPS reporting for BSHD-layout kernels [batch, seq, heads, dim].
...
// Choreo flash kernels in this tree use BHSD device layout. Host tensors stay
// BSHD; transpose in/out around each launch (included in timing).
inline void transpose_bshd_to_bhsd(const choreo::f16* src, choreo::f16* dst,
...
inline void transpose_bhsd_to_bshd(const choreo::f16* src, choreo::f16* dst,
                                   size_t B, size_t seq_len, size_t H,
                                   size_t head_dim) {
  for (size_t b = 0; b < B; ++b) {
    for (size_t h = 0; h < H; ++h) {
...
struct HostBhsdBuffers {
  std::vector<choreo::f16> q;
  std::vector<choreo::f16> k;
  std::vector<choreo::f16> v;
  std::vector<choreo::f16> o;
};
...
inline void download_output_bshd(choreo::spanned_data<choreo::f16, 4>& O_h,
                                 choreo::f16* o_d, HostBhsdBuffers& host) {
  const size_t B = O_h.shape()[0];
  const size_t Q_SEQ = O_h.shape()[1];
  const size_t H = O_h.shape()[2];
  const size_t head_dim = O_h.shape()[3];
  const size_t bytes = B * H * Q_SEQ * head_dim * sizeof(choreo::f16);

  choreo::abend_true(
      cudaMemcpy(host.o.data(), o_d, bytes, cudaMemcpyDeviceToHost));
  choreo::abend_true(cudaDeviceSynchronize());
  transpose_bhsd_to_bshd(host.o.data(), O_h.data(), B, Q_SEQ, H, head_dim);
}
...
inline int RunBenchmarks(const char* title, BenchConfig* configs,
  ...
  std::cout << "Choreo kernel: BHSD device tiles; host BSHD<->BHSD once per config\n";
  std::cout << "Timing: device kernel only (same as flash_mha_fwd_bhsd bench)\n";
  if (do_timing) {
  ...
  for (size_t ci = 0; ci < num_configs; ++ci) {
    ...
    // Kernel-only timed path (matches bhsd mha_helper; excludes host transpose/H2D).
    auto launch_kernel = [&]() {
      kernel_fn(cfg, views);
      choreo::abend_true(cudaDeviceSynchronize());
    };

    auto fetch_output_bshd = [&]() {
      const size_t q_bytes = B * H * Q_SEQ * head_dim * sizeof(choreo::f16);
