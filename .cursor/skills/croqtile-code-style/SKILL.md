...
## Section 5: Build and Compilation Reference

Primary workflow in Croqtile: **Make** driving CMake and Ninja.

Typical targets (use the project Makefile as source of truth):

- `make` -- default build
- `make debug` -- debug configuration
- `make release` -- optimized build
- `make clean` -- clean artifacts
- `make format` -- run `clang-format` using project config
- `make test` -- run tests

Prefer these Make targets over hand-rolled compiler invocations for day-to-day work.

### CMake reference (optional)

Useful when generating `compile_commands.json` or inspecting generator output:
...
Debug and instrumentation flags:
...
### Example Makefile pattern (reference)
