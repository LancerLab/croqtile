...
- **All build operations must go through the top-level `Makefile` entrypoint**; do not run `cmake` or `ninja` directly for normal workflow.
- Build artifacts `./choreo` and `./copp` are **symlinks** to binaries under `build/` (or `build-debug/` / `build-release/` for those variants).
- The default compilation target is `cute` (CUDA CuTe).
- **Never write generated or intermediate files under `/tmp`**. Place temporary outputs under the current workspace (for example `build/`).
- **Attach an explicit timeout** to execution commands. Use a conservative default (at least 30 seconds for light runs; increase for heavy compile or execute steps).

### Environment

- Ensure CUDA is available (commonly `/usr/local/cuda`) and CuTe/CUTLASS is available at `extern/cutlass` unless overridden.
- `CUDA_HOME` defaults to `/usr/local/cuda` when relevant. Override if needed.
...
After a successful build, `./choreo` and `./copp` symlinks are created or updated automatically.
...
Quickly verify the `./choreo` symlink:
...
- If `.cpp` / `.hpp` files under `lib/` (or compiler-related `tools/`) changed, run `make` (or `make build`) again.
- On compile errors or broken links, try `make clean` followed by `make`.
