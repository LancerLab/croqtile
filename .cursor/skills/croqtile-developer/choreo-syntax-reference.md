...
This inventory is derived from observed `.co` files in `tests/` and `benchmark/`, plus project rules for preferred new syntax. Use it to match existing syntax, then verify against nearby files in the same directory.
...
- `chunkat(...)`
  ...
  - Examples: `tests/gpu/end2end/add.co`, `tests/infer/view_from.co`
- `view(...).from(...)`
...
- `subspan(...).at(...)`
  ...
  - Examples: `tests/infer/subspan.co`, `benchmark/performance/matmul/matmul_f16_dyn_sm90.co`
- `subspan(...).step(...).at(...)`
...
- `tests/gpu/end2end/tma_v2.co` - `tma.copy(.async)` and `subspan`
- `tests/gpu/end2end/matmul/matmul_f16_basic.co` - `tma`, `mma`, `: block`, `: group-4`
- `tests/norm/normalize.co` - `span_as`, `dma.transp`, decomposed indices
- `tests/gpu/end2end/add.co` - `span_as`, `chunkat`, host `main()`
...
- `benchmark/performance/blockscale_gemm/blockscale_gemm_dyn_sm90.co` - blockscaled GEMM with scale operands and tensor-core pipeline
...
- GPU kernel style or directory convention is mixed.
...
- Prefer examples that share the same GPU generation marker in comments or path, such as `sm86`, `sm90`, or `sm90a`.
