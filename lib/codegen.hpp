...
struct DeviceCodeGen {
  ...
  // File extension for device source generated by the target CodeGenerator.
  virtual std::string SourceExtension() const { return ".cpp"; }

  // Per-target subdirectory for device-specific headers and build artifacts.
  std::string TargetBuildDir(const std::string& build_path) const {
    return build_path + "/" + TargetName();
  }

  // Emit additional setup files (e.g. target-specific header defines) needed
  // to compile device source in the given build directory.
  virtual void EmitSetupFiles(std::ostream&, const std::string&) const {}
  ...
  // Compile a device source file (.cu/.cpp) with the device toolchain.
  // Default: same as EmitHostCompileCommand.
  virtual void EmitDeviceCompileCommand(std::ostream& out,
                                        const std::string& build_path,
                                        const std::string& src,
                                        const std::string& obj) const {
    EmitHostCompileCommand(out, build_path, src, obj);
  }
  ...
  virtual void EmitLinkCommand(std::ostream& out,
    ...
    out << R"script(  CXX_LIBDIR="$(${CXX:-g++} -print-file-name=libstdc++.so | xargs dirname | xargs realpath 2>/dev/null)"
  RPATH_FLAG=""
  if [ -n "$CXX_LIBDIR" ] && [ -d "$CXX_LIBDIR" ]; then
    ...
    out << " " << LinkFlags() << " -lm -lpthread ${RPATH_FLAG} -o " << exe_file
