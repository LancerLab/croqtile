...
void TopsccCodeGen::EmitLibCall(AST::Call& n, const std::string& func_name,
  ...
  if (kind == LibCallKind::GEMM) {
    ...
    if (has_bias)
      ...
      bias_str =
          std::string("(") + NameBaseType(lhs_bt, false) + "*)nullptr";
    else
      ...
      if (info.use_acore) {
        ...
        if (info.use_static_M) {
          os << indent << "  acore::matmul<" << info.static_M
             << ", acore::MK_KN>(" << out_str << ", " << a_str << ", "
             << b_str << ", " << bias_str << ", __lib_gemm_ws__, " << k_str
             << ", " << n_str << ", 0, 1, " << (has_bias ? "1" : "0")
             << ", 0, 0);\n";
        } else {
    ...
    if (CCtx().UseTargetLib()) {
      ...
      std::string type_str = NameBaseType(
          bt == BaseType::UNKNOWN ? BaseType::F32 : bt, true);
    ...
    if (CCtx().UseTargetLib()) {
      ...
      auto cast = DeduceElemCast(0);
      auto bt = DeduceBaseType(0);
      std::string data_type = NameBaseType(
          bt == BaseType::UNKNOWN ? BaseType::F32 : bt, false);
      std::string cal_type =
          (bt == BaseType::F32 || bt == BaseType::F16 ||
      ...
      os << indent << "  acore::" << acore_name << "<0, " << data_type
         << ", " << cal_type << ">(" << cast << dst << ", " << cast
         << src << ", __reduce_ws__, (" << data_type << ")" << init_val
         << ", 1, " << num << " / " << nred << ", " << nred
         << ", true, true);\n";
      os << indent << "}\n";
    } else {
    ...
    if (CCtx().UseTargetLib()) {
      ...
      os << indent << "  acore::addmm<acore::MK_KN>(" << out_s << ", "
         << a_s << ", " << b_s << ", " << bias_s
         << ", __lib_addmm_ws__, " << m_s << ", " << k_s << ", " << n_s
         << ", 0, " << alpha_s << ", " << beta_s << ");\n";
      os << indent << "}\n";
    } else {
      Warning(n.LOC(), "'" + func_name +
      ...
      std::string type_str = NameBaseType(
          lhs_bt == BaseType::UNKNOWN ? BaseType::F32 : lhs_bt, true);
      os << indent << "choreo::lib_addmm<" << type_str << ">("
         << out_s << ", " << bias_s << ", " << a_s << ", " << b_s << ", "
         << m_s << ", " << k_s << ", " << n_s << ", " << alpha_s << ", "
         << beta_s << ");\n";
    }
    ...
    if (CCtx().UseTargetLib()) {
      has_acore_call = true;
      auto cast = DeduceElemCast(0);
      os << indent << "acore::layer_norm(" << cast << dst << ", "
         << cast << src << ", " << cast << weight << ", " << cast << bias
         << ", " << batch << ", " << norm_size << ", " << eps << ");\n";
    } else {
      Warning(n.LOC(), "'" + func_name +
                           "' using loop-based general fallback "
      ...
      std::string type_str =
          NameBaseType(bt == BaseType::UNKNOWN ? BaseType::F32 : bt, true);
      os << indent << "choreo::lib_layer_norm<" << type_str << ">("
         << dst << ", " << src << ", " << weight << ", " << bias << ", "
         << batch << ", " << norm_size << ", " << eps << ");\n";
    }
    ...
    if (CCtx().UseTargetLib()) {
      ...
      os << indent << "  int __wo = (" << wi_s << " + 2*" << pw_s << " - "
         << kw_s << ") / " << sw_s << " + 1;\n";
      os << indent << "  acore::conv2d<__ho, __wo>(" << out_s << ", "
         << inp_s << ", " << wgt_s << ", " << bias_s << ", " << batch_s
         << ", " << hi_s << ", " << wi_s << ", " << ci_s << ", " << kh_s
         << ", " << kw_s << ", " << co_s << ", __ho, __wo, " << sh_s
         << ", " << sw_s << ", 1, 1, 1, 0, 0);\n";
      os << indent << "}\n";
    } else {
      Warning(n.LOC(), "'" + func_name +
      ...
      std::string type_str =
          NameBaseType(bt == BaseType::UNKNOWN ? BaseType::F32 : bt, true);
      os << indent << "choreo::lib_conv2d<" << type_str << ">("
         << out_s << ", " << inp_s << ", " << wgt_s << ", " << bias_s
         << ", " << batch_s << ", " << hi_s << ", " << wi_s << ", "
         << ci_s << ", " << co_s << ", " << kh_s << ", " << kw_s << ", "
         << sh_s << ", " << sw_s << ", " << ph_s << ", " << pw_s
         << ");\n";
    ...
    if (CCtx().UseTargetLib()) {
      ...
      os << indent << "acore::convert(" << dst_cast << dst << ", "
         << src_cast << src << ", " << num << ");\n";
    } else {
      Warning(n.LOC(), "'" + func_name +
                           "' using loop-based general fallback "
