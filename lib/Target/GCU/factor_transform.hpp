...
public:
  FactorTrans() : VisitorWithSymTab("ftran") {}
  ~FactorTrans() {}
  ...
  bool Visit(AST::Expr& n) {
    ...
    if (n.op != Op::DataOf && n.op != Op::MDataOf) return true;
  ...
  bool Visit(AST::Assignment& n) {
    ...
    // do not care about the swap generated one
    if (SuffixedWith(n.GetName(), SWAP_SFX_PRE)) return true;
    if (SuffixedWith(n.GetName(), SWAP_SFX_POS)) return true;
  ...
  bool Visit(AST::ForeachBlock& n) {
    ...
    if (lbound == nullptr) lbound = AST::MakeIntExpr(n.LOC(), 0);
