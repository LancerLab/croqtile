...
void SemaChecker::CheckChunkAtTileBounds(AST::ChunkAt& n, AST::DMA& dma) {
  ...
  for (auto& op : n.AllOperations()) {
    ...
    for (size_t d = 0; d < indices->Count(); ++d) {
      ...
      ptr<AST::Node> index_expr = index_node;
      if (auto ii = dyn_cast<AST::IntIndex>(index_node))
        index_expr = ii->Val();
      ValueItem index = GetInvalidValueItem();
      if (auto expr = dyn_cast<AST::Expr>(index_expr)) {
        if (expr->Opts().HasVal()) index = expr->Opts().GetVal();
      } else if (auto id = dyn_cast<AST::Identifier>(index_expr)) {
        index = sbe::sym(InScopeName(id->name));
      } else if (auto lit = dyn_cast<AST::IntLiteral>(index_expr)) {
        index = sbe::nu(lit->Val());
      }
      if (!IsValidValueItem(index) || !IsComputable(index)) continue;

      auto factor_node = factors->ValueAt(d);
      ptr<AST::Node> factor_expr = factor_node;
      if (auto ii = dyn_cast<AST::IntIndex>(factor_node))
        factor_expr = ii->Val();
      ValueItem tile_count = GetInvalidValueItem();
      if (auto expr = dyn_cast<AST::Expr>(factor_expr)) {
        if (expr->Opts().HasVal()) tile_count = expr->Opts().GetVal();
      } else if (auto id = dyn_cast<AST::Identifier>(factor_expr)) {
        tile_count = sbe::sym(InScopeName(id->name));
      } else if (auto lit = dyn_cast<AST::IntLiteral>(factor_expr)) {
