...
| Option | Description |
|---|---|
| `-t <target>` | Set compilation target: `cute` (GPU), `factor` (GCU), `topscc` (GCU) |
...
2. Analyze .co file
   ...
   └─ Content clues:
       ...
       └─ includes `choreo.h`, `__cok__` → factor/topscc target

3. Determine target
   ├─ path contains gpu/cuda/cute → cute
   ├─ path contains factor        → factor
   ├─ path contains topscc        → topscc
...
1. **Unknown target** → ask: "Please choose target: cute / factor / topscc"
