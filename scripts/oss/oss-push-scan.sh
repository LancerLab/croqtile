...
usage() {
  cat <<'EOF'
Usage: oss-push-scan.sh [options]
...
scan_paths() {
  # Reads path list from stdin; one grep call replaces N subshell forks.
  local ctx="$1"
  local matches
  matches="$(grep -E $CASE_FLAG -- "$COMBINED_PATTERN" 2>/dev/null || true)"
  [[ -z "$matches" ]] && return
  while IFS= read -r fpath; do
    [[ -n "$fpath" ]] && record "$ctx/path" "$fpath"
  done <<< "$matches"
}
...
filter_excluded_lines() {
  # Replaces the per-line bash loop with a single awk pass.
  # Input lines: "treeish:path:lineno:content" (with prefix) or "path:lineno:content".
  local plen="${#FILTER_TREEISH_PREFIX}"
  awk -v plen="$plen" -v re="$EXCL_PATH_RE" '
    length($0) == 0 { next }
    {
      s = (plen > 0) ? substr($0, plen + 1) : $0
      n = index(s, ":")
...
scan_content_via_git_grep() {
  # Pass :(exclude) pathspecs directly to git grep so excluded paths are
  # never read at all -- no post-filter pass needed.
  local treeish="$1"
  local ctx="$2"
  local matches
  matches="$(git -C "$REPO_ROOT" grep -I -nE $CASE_FLAG -- "$COMBINED_PATTERN" "$treeish" \
    -- ${EXCL_PATHSPECS[@]+"${EXCL_PATHSPECS[@]}"} 2>/dev/null || true)"
  [[ -z "$matches" ]] && return
  local count
...
scan_nonascii_via_git_grep() {
  ...
  local ctx="$2"
  local matches
  matches="$(git -C "$REPO_ROOT" grep -I -nP -- "$NON_ASCII_RE" "$treeish" \
    -- ${EXCL_PATHSPECS[@]+"${EXCL_PATHSPECS[@]}"} 2>/dev/null || true)"
  [[ -z "$matches" ]] && return
  local count
  count="$(echo "$matches" | wc -l)"
...
# Build a single ERE pattern for fast grep-based path exclusion, and an
# array of git :(exclude) pathspecs for native git path filtering.
# Uses pure-bash string replacement (no sed subprocesses) for fast startup.
EXCL_PATH_RE=""
EXCL_PATHSPECS=()
for _d in "${EXCL_DIRS[@]}"; do
  [[ -n "$EXCL_PATH_RE" ]] && EXCL_PATH_RE+="|"
  _esc="${_d//./\\.}"
  EXCL_PATH_RE+="^${_esc}"
  EXCL_PATHSPECS+=(":!${_d}**")
done
for _e in "${EXCL_EXACT[@]}"; do
  [[ -n "$EXCL_PATH_RE" ]] && EXCL_PATH_RE+="|"
  _esc="${_e//./\\.}"
  EXCL_PATH_RE+="^${_esc}\$"
  EXCL_PATHSPECS+=(":!${_e}")
done
for _g in "${EXCL_GLOBS[@]}"; do
  [[ -n "$EXCL_PATH_RE" ]] && EXCL_PATH_RE+="|"
  _esc="${_g//./\\.}"
  _esc="${_esc//\*/.*}"
  EXCL_PATH_RE+="^${_esc}\$"
...
scan_coupled_changes() {
  ...
  local all_files
  mapfile -t all_files < <(git -C "$REPO_ROOT" diff-tree --no-commit-id -r --diff-filter=d --name-only "$rev")
...
filter_excluded_paths() {
  # Single grep -Ev pass replaces per-line bash loop.
  grep -Ev -- "$EXCL_PATH_RE" 2>/dev/null || true
}
...
mode_staged() {
  ...
  scan_paths "staged" < <(git -C "$REPO_ROOT" diff --cached --diff-filter=d --name-only)
...
mode_diff() {
  ...
  scan_paths "diff" < <(git -C "$REPO_ROOT" diff-tree --no-commit-id -r --diff-filter=d --name-only "$rev")
...
mode_worktree() {
  ...
  # Scan contents using git grep with native path exclusion
  local matches
  matches="$(git -C "$REPO_ROOT" grep -I -nE $CASE_FLAG -- "$COMBINED_PATTERN" \
    -- ${EXCL_PATHSPECS[@]+"${EXCL_PATHSPECS[@]}"} 2>/dev/null || true)"
  if [[ -n "$matches" ]]; then
    local count
    count="$(echo "$matches" | wc -l)"
  ...
  # Strict non-ASCII check on worktree with native path exclusion
  local na_matches
  na_matches="$(git -C "$REPO_ROOT" grep -I -nP -- "$NON_ASCII_RE" \
    -- ${EXCL_PATHSPECS[@]+"${EXCL_PATHSPECS[@]}"} 2>/dev/null || true)"
