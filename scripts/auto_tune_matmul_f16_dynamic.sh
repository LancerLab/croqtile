...
CO_FILE_DEFAULT="benchmark/performance/matmul/matmul_f16_dynamic.co"
ARCH="sm_90a"
GPU_INDEX=""
WARMUP="5"
REPEAT="20"
...
show_usage() {
  cat <<'EOF'
Usage:
  scripts/auto_tune_matmul_f16_dynamic.sh [options]

Options:
  --co <file>           .co source path (default: benchmark/performance/matmul/matmul_f16_dynamic.co)
  ...
  --out-dir <dir>       output directory (default: build/auto_tune/matmul_f16_dynamic_tune_<ts>)
  -h, --help            show help
EOF
}
...
if [[ -z "$OUT_DIR" ]]; then
  OUT_DIR="build/auto_tune/matmul_f16_dynamic_tune_$(date +%Y%m%d_%H%M%S)"
fi
mkdir -p "$OUT_DIR"
...
# SM90 WGMMA f16/f16 -> f16/f32 supports shapes: m=64, n in [8,256] step 8, k=16.
# For this kernel, WARP_K controls the micro-k shape (fixed to 16), while TILE_K/SWIZ
# controls K staging depth and swizzle layout.
WARP_M_CANDIDATES=(64)
WARP_N_CANDIDATES=()
for n in $(seq 8 8 256); do
  # skip TN=96 which is known to crash on some GPUs
  if [[ "$n" -eq 96 ]]; then
    continue
  fi
  WARP_N_CANDIDATES+=("$n")
done
WARP_K_CANDIDATES=(16)
# TILEK_SWIZ_PAIRS=("64:128" "32:64" "16:32")
TILEK_SWIZ_PAIRS=("64:128")
...
for pair in "${TILEK_SWIZ_PAIRS[@]}"; do
  TILE_K="${pair%%:*}"
  SWIZ="${pair##*:}"
  for WARP_M in "${WARP_M_CANDIDATES[@]}"; do
    for WARP_K in "${WARP_K_CANDIDATES[@]}"; do
      for WARP_N in "${WARP_N_CANDIDATES[@]}"; do
    NAME="w${WARP_M}n${WARP_N}k${WARP_K}_tk${TILE_K}_sw${SWIZ}"
    TILE_M=$WARP_M
    TILE_N=$WARP_N
...
if [[ -z "$BEST_NAME" ]]; then
  # ensure an output file even if all candidates failed
  {
  ...
  {
    echo "# matmul_f16_dynamic Auto-tune Summary"
    echo
