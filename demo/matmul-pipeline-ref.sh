...
void __choreo_ele_add() {
  ...
  D(func_)("__choreo_ele_add", {lhs_type, rhs_type}, {output_type}, [&](auto args, auto results) {
    ...
    auto lhs_load_s_buffer = alloc_(L1Type(IntType(32),{256, 256}));
    auto lhs_load_s = alloc_dma_(SDMAType());
    auto rhs_load_s_buffer = alloc_(L1Type(IntType(32),{256, 256}));
    auto rhs_load_s = alloc_dma_(SDMAType());
    auto out_load_s_buffer = alloc_(L1Type(IntType(32),{256, 256}));
    ...
    for_(m_tile, 1, 1, [&](auto iv_m_tile) {
      for_(n_tile, 1, 1, [&](auto iv_n_tile) {
        ...
        for_(k_tile, 1, 1, [&](auto iv_k_tile) {
          ...
          n_tile_s = 0;
          for_(m_tile_s, 2, 1, [&](auto iv_m_tile_s) {
            for_(n_tile_s, 4, 1, [&](auto iv_n_tile_s) {
              for_(k_tile_s, 4, 1, [&](auto iv_k_tile_s) {
                async_load_(out_load_s, l2_out, out_load_s_buffer, {Value(256)*iv_m_tile_s,Value(256)*iv_n_tile_s});
                async_load_(lhs_load_s, lhs_load_buffer, lhs_load_s_buffer, {Value(256)*iv_m_tile_s,Value(256)*iv_k_tile_s}).wait_on_(lhs_load);
                async_load_(rhs_load_s, rhs_load_buffer, rhs_load_s_buffer, {Value(256)*iv_k_tile_s,Value(256)*iv_n_tile_s}).wait_on_(rhs_load);
                // async_load_(lhs_load_s, lhs_load_buffer, lhs_load_s_buffer, {Value(256)*iv_m_tile_s,Value(256)*iv_k_tile_s});
                // async_load_(rhs_load_s, rhs_load_buffer, rhs_load_s_buffer, {Value(256)*iv_k_tile_s,Value(256)*iv_n_tile_s});
                // wait_dma_(lhs_load_s);
                // wait_dma_(rhs_load_s);
                wait_dma_(out_load_s);
                call_("dot_general_fp32", {lhs_load_s_buffer.addr_(),rhs_load_s_buffer.addr_(),256,256,256,0,8,out_load_s_buffer.addr_()});
		            async_store_(out_store_s, out_load_s_buffer, l2_out, {Value(256)*iv_m_tile_s,Value(256)*iv_n_tile_s}).notify_(out_store);
		            // async_store_(out_store_s, out_load_s_buffer, l2_out, {Value(256)*iv_m_tile_s,Value(256)*iv_n_tile_s});
